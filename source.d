...
void main()
{
    ...
    // here we create the GUI window, we will use create2dWindow() rather than SimpleWindow() because of the OpenGl features that SimpleWindow() doesn't support
    ...
    void damageAliensIfYouHitThemOrLoseIfYouCrashIntoThem()
    {
        ...
        foreach (i; 0 .. 6)
        {
            ...
            if (!aliens[i].destroyed && aliens[i].position.y + aliens[i].alive.height() > 0 && aliens[i].position.y < 400)
            {
				...
				// here we check each prime fire you've shot to see if it hit an alien, in which case it must disappear, for that we must shorten the array of
    ...
    // here we start the audio thread and start playing the background music
